...
class MqttGen {
     ...
     */
    const S_MQTT = 'mqtt';       # mqtt section
    const S_MISC = 'misc';       # miscellaneous section
    const S_MESSAGES = 'messages';   # topics definition section
    const S_FUN = 'func';        # function name
    const S_PAR = 'param';       # parameters associated to the function
    const S_TOPIC = 'topic';     # topic name
    const S_PAYLOAD = 'payload'; # payload definition
    const S_SET = 'set:';        # trailing string to define set command topics mqttgen shall subscribe to
    const S_GET = 'get';         # get key
    const S_SYNC = 'sync';       # synchronous boolean status. If not present, true is assumed.
    const S_ONCE = 'once';       # If true payload is sent only once after connexion. false by default.
    const S_RETAIN = 'retain';   # To specify a retain message. false by default.
     ...
     *
     * @param $mqtt_cnfg configuration parameters
     */
	...
	if ($will_msg != '') {
	    $this->logger->info("set will message: topic=" . $will_topic . ", payload=" . $will_msg .
     ...
     *
     * @param $key JSON key of the given message structure
     * @param $msg JSON array defining the message
     * @param $is_async true to send the message whatever the value of S_SYNC
	...
	if ($typ == 'array') {
		...
		$gen_data = call_user_func_array(
		    __NAMESPACE__ . '\\MqttGenFunc::' . $data[self::S_FUN],
     ...
     * Check message definition and subscribe to get and set related topics
     *
     * @param $key key of the given data structure
     * @param $data JSON data array or value
     */
	...
	if ($typ == 'array') {
		...
		if ($i === self::S_TOPIC) {
			...
			$this->logErrorAndThrowException('key "' . (self::S_PAR)  . '", or element "' .
                                                         (self::S_PAR) . '[' . $value . ']" not found in element ' . $key);
